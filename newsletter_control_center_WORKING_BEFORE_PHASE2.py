...
def render_user_profile_selector() -> Path:
    ...
    selected = st.sidebar.selectbox("Active user", list(labels.keys()))
    profile = get_user_profile(labels[selected])
        ...
        if st.button("Create Profile"):
            if new_name.strip():
                new_profile = create_user_profile(
                    ...
                    copy_from_portfolio=profile.portfolio_file,
                )
...
def render_overview(df: pd.DataFrame, portfolio_path: Path) -> None:
    ...
    st.info(
        "This app is designed as a facts-first daily portfolio update system. "
        "It monitors holdings, exposures, filings, catalysts, alerts, and newsletter output. "
        "It does not make investment decisions for the user."
    )
...
def render_construction() -> None:
    ...
    st.caption("Internal advanced portfolio construction workflow. Hidden from the main published navigation.")
...
def render_execution() -> None:
    ...
    st.caption("Internal advanced execution workflow. Hidden from the main published navigation.")
...
def render_backtesting() -> None:
    ...
    st.caption("Internal advanced backtesting workflow. Hidden from the main published navigation.")
...
def render_monitor() -> None:
    ...
    try:
        ...
        with st.spinner("Loading factual holdings monitor..."):
            ...
            result = build_holdings_change_monitor(
                holdings,
        ...
        with c1:
            st.metric("Total Value", f"${exposure.get('total_value', 0):,.2f}")

        with c2:
            st.metric("Largest Position", f"{exposure.get('single_name_concentration_pct', 0):.2f}%")

        with c3:
            st.metric("Top 3 Concentration", f"{exposure.get('top_3_concentration_pct', 0):.2f}%")

        with c4:
            st.metric("Portfolio Beta", f"{exposure.get('portfolio_beta', 0):.2f}")

        e1, e2 = st.columns(2)

        with e1:
            sector_df = pd.DataFrame(exposure.get("sector_exposure", []))
            st.markdown("##### Sector Exposure")
            if not sector_df.empty:
                chart = sector_df.set_index("sector")[["weight_pct"]].rename(columns={"weight_pct": "Weight (%)"})
                st.bar_chart(chart, height=280)
                st.dataframe(_format_table_for_display(sector_df), use_container_width=True, hide_index=True)

        with e2:
            cap_df = pd.DataFrame(exposure.get("market_cap_exposure", []))
            st.markdown("##### Market Cap Exposure")
            if not cap_df.empty:
                chart = cap_df.set_index("market_cap_tier")[["weight_pct"]].rename(columns={"weight_pct": "Weight (%)"})
                st.bar_chart(chart, height=280)
                st.dataframe(_format_table_for_display(cap_df), use_container_width=True, hide_index=True)

        e3, e4 = st.columns(2)

        with e3:
            style_df = pd.DataFrame(exposure.get("style_exposure", []))
            st.markdown("##### Growth / Value Tilt")
            if not style_df.empty:
                chart = style_df.set_index("style")[["weight_pct"]].rename(columns={"weight_pct": "Weight (%)"})
                st.bar_chart(chart, height=280)
                st.dataframe(_format_table_for_display(style_df), use_container_width=True, hide_index=True)

        with e4:
            region_df = pd.DataFrame(exposure.get("region_exposure", []))
            st.markdown("##### Geographic Exposure")
            if not region_df.empty:
                chart = region_df.set_index("region")[["weight_pct"]].rename(columns={"weight_pct": "Weight (%)"})
                st.bar_chart(chart, height=280)
                st.dataframe(_format_table_for_display(region_df), use_container_width=True, hide_index=True)

        st.divider()
        st.markdown("#### Holdings Diagnostics")

        if holdings_df.empty:
            st.info("No holdings monitor rows available.")
            return
        ...
        display_cols = [
            ...
            "weekly_move_pct",
            "weekly_vs_spy_pct",
            "weekly_vs_sector_etf_pct",
            "earnings_proximity",
            "recent_sec_filing",
            "volume_vs_30d_avg",
            "distance_from_52w_high_pct",
            "drawdown_from_63d_high_pct",
            "market_cap_tier",
            "style",
            "beta",
            "region",
        ...
        clean = holdings_df[display_cols].rename(columns={
            ...
            "daily_move_pct": "Daily Move (%)",
            "weekly_move_pct": "Weekly Move (%)",
            "weekly_vs_spy_pct": "Weekly vs SPY (%)",
            "weekly_vs_sector_etf_pct": "Weekly vs Sector ETF (%)",
            "earnings_proximity": "Earnings Proximity",
            "recent_sec_filing": "Recent SEC Filing",
            "volume_vs_30d_avg": "Volume vs 30D Avg (x)",
            "distance_from_52w_high_pct": "Distance from 52W High (%)",
            "drawdown_from_63d_high_pct": "Drawdown from 63D High (%)",
...
def render_market_workbench() -> None:
    ...
    st.caption("Facts-first investor tools: catalysts, SEC filing changes, watchlist monitoring, portfolio drift, and macro dashboard.")
    ...
    try:
        ...
        from app.data.earnings import get_upcoming_earnings
        from app.data.sec_filings import get_sec_filings
        from app.data.market_workbench import catalyst_calendar, sec_diff, watchlist_monitor, portfolio_drift, macro_dashboard, exposure_dashboard
        ...
        with st.spinner("Loading factual market workbench..."):
        ...
        tabs = st.tabs(["What Matters This Week", "SEC Filing Diff Tracker", "Watchlist", "Portfolio Drift", "Macro Dashboard", "Exposure"])

        with tabs[0]:
            st.markdown("#### Earnings & Catalyst Calendar")
            st.dataframe(_format_table_for_display(catalysts), use_container_width=True, hide_index=True)

        with tabs[1]:
            st.markdown("#### SEC Filing Diff Tracker")
            st.caption("Tracked factual language categories from recent filings. No recommendation layer.")
            st.dataframe(_format_table_for_display(sec_rows), use_container_width=True, hide_index=True)

        with tabs[2]:
            st.markdown("#### Watchlist Intelligence")
            st.dataframe(_format_table_for_display(watch), use_container_width=True, hide_index=True)

        with tabs[3]:
            st.markdown("#### Portfolio Drift Monitor")
            if not drift.empty:
                chart = drift.set_index("Ticker")[["Current Weight (%)", "Target Weight (%)"]]
                st.bar_chart(chart, height=320)
            st.dataframe(_format_table_for_display(drift), use_container_width=True, hide_index=True)
        ...
        with tabs[4]:
            ...
            st.dataframe(_format_table_for_display(indicators), use_container_width=True, hide_index=True)
        ...
        with tabs[5]:
            ...
            if exposure:
                ...
                for label in ["Sector Exposure", "Market Cap Exposure", "Style Exposure", "Region Exposure"]:
                    st.markdown(f"##### {label}")
                    df = pd.DataFrame(exposure.get(label, []))
                    if not df.empty:
                        idx_col = df.columns[0]
                        st.bar_chart(df.set_index(idx_col)[["Weight (%)"]], height=260)
                        st.dataframe(_format_table_for_display(df), use_container_width=True, hide_index=True)
